...
describe('Parsing smooth curve to commands', () => {
  test('should not work when badly declared', () => {
    expect(() => new SVGPathData('S')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('S10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('S10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('S10 10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('S10 10 10 10 10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('S10 10 10S10 10 10 10')).toThrow(
      new SyntaxError('Unterminated command at index 9.'),
    );
  });

  test('should work with comma separated coordinates', () => {
    const commands = new SVGPathData('S123,456 789,987').commands as CommandS[];
  ...
  test('should work with nested separated complexer coordinate pairs', () => {
    const commands = new SVGPathData(
      'S-10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5',
  ...
  test('should work with multiple pairs of coordinates', () => {
    const commands = new SVGPathData(
      'S-10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5 -10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5 -10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5',
    ).commands as CommandS[];
  ...
  test('should work with multiple declared pairs of coordinates', () => {
    const commands = new SVGPathData(
      'S-10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5 s-10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5 S-10.0032e-5,-20.0032e-5 -30.0032e-5,-40.0032e-5',
    ).commands as CommandS[];
