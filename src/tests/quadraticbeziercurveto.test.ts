...
describe('Parsing quadratic bezier curve to commands', () => {
  test('should not work when badly declared', () => {
    expect(() => new SVGPathData('Q')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('Q10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('Q10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('Q10 10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('Q10 10 10 10 10 10')).toThrow(
      new SyntaxError('Unterminated command at the path end.'),
    );

    expect(() => new SVGPathData('Q10 10 10Q10 10 10 10')).toThrow(
      new SyntaxError('Unterminated command at index 9.'),
    );
  });

  test('should work with comma separated coordinates', () => {
    const commands = new SVGPathData('Q123,456 789,987').commands as CommandQ[];
...
describe('Encoding line to commands', () => {
  test('should work with one command', () => {
    ...
    ).toEqual('Q-0.000100032 -0.000200032 -0.000300032 -0.000400032');
  });
